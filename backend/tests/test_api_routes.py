...
def test_forecast_rates_default_shock(client):
    mock_response = {
        "forecast": [{"date": "2026-09-08", "p10": 1000, "p50": 1200, "p90": 1400}],
        "provenance": {
            "source": "test",
            "mode": "demo",
    ...
    with patch(
        "app.services.ml_predictor.get_freight_forecast", new_callable=AsyncMock
    ) as mock_forecast:
...
def test_market_ticker_endpoint(client):
    mock_market = [
        {"symbol": "BDRY", "name": "Baltic Dry Index", "price": 1000, "change_pct": 1.2}
    ]
    with patch(
        "app.api.routers.market.get_market_ticker", new_callable=AsyncMock
    ) as mock_get_market:
