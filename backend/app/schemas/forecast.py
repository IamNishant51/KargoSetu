...
class ForecastRateResponse(BaseModel):
    ...
    p10: float = Field(
        ...,
        description="10th percentile freight rate prediction (lower bound)",
        examples=[1450.25],
    )
    p50: float = Field(
        ...,
        description="50th percentile freight rate prediction (median)",
        examples=[1500.0],
    )
    p90: float = Field(
        ...,
        description="90th percentile freight rate prediction (upper bound)",
        examples=[1550.75],
    )


class ForecastResponse(BaseModel):
    forecast: list[ForecastRateResponse]
