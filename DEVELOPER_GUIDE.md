...
This is the 3D "Gods Eye View" of the Bay of Bengal. It is lazy-loaded so the landing page stays fast: only the globe route ever imports Cesium (resolved version 1.145.0, `^1.124.0` series per plan). Cesium workers live in `public/cesium/` (copied by `scripts/copy-cesium.mjs` on postinstall) and the globe sets `CESIUM_BASE_URL` to `/cesium` before building the Viewer. Cesium is loaded via a `<script>` tag (`/cesium/Cesium.js`) through the `getCesium()` singleton in `components/map/api.ts` — never bundled, because bundling its Source tree lets binary-adjacent strings through the minifier as illegal octal escapes and kills the 3D chunk. Default map is keyless Esri World Imagery with automatic OSM fallback; terrain is flat ellipsoid (the public quantized-mesh endpoint 404s and this Cesium build has no ArcGIS terrain provider, so mesh attempts only produced console errors — and ocean corridor needs no relief). No Google tiles and no committed keys.
...
* **Map components**: `KargoGlobe.tsx` (Viewer lifecycle: create once, destroy on unmount; imports Cesium `widgets.css` inside the lazy chunk so the canvas and toolbar size correctly; frames the corridor instantly with `setView`, resizes via ResizeObserver, fades in on ready; default help/infobox/selection widgets off, home button resets to the corridor overview; performance-first canvas (pixel ratio capped at 1.5x, MSAA 2x) with standalone `flyCameraTo` helper), `VesselLayer.tsx` (one true 3D ship model per vessel from `public/models/ship.glb`, interpolated between 30s polls with zero per-frame allocation, heading quaternion from cog, draft-limit red ring, labels on selection/detection only, capped at 500), `HazardLayer.tsx` (quake circles with crimson badge pills, fire dots, weather wash and crisp cyan tactical weather badge), `CorridorLayer.tsx` (Newcastle to Sandheads to Haldia/Paradip/Dhamra lines plus port markers styled as high-contrast tactical pills with individual non-overlapping offsets), `VesselSheet.tsx` (click a ship, see its data, press Evaluate to call the same solver the desk uses), `LayerToggles.tsx`, `Hud.tsx` (camera and counts, max 4Hz), `TourDirector.tsx` (5-stop SIH stage tour, Esc exits), `SensorStyles.tsx` (Normal/CRT/NVG/FLIR via CSS overlay, keys 1-4), `CommandBar.tsx` (text commands with intelligent natural-language parser supporting "take me to haldia", aliases, punctuation stripping, and quick clickable navigation chips).
* **Landing Mini Globe (`components/landing/CorridorMiniGlobe.tsx`)**: An interactive, responsive 3D corridor radar embedded directly into `PortCorridor.tsx` on the landing page (with a toggle back to the schematic chart). Features live AIS vessels, fairway corridors, weather condition badges, clickable vessel inspector with direct link to cargo solver, and quick-fly camera presets, all lazy-loaded with `next/dynamic` for zero impact on initial landing bundle!
...
* **Credits that never hide**: `attribution.ts` holds one shared line used by both the in-app popover and the landing footer, so they cannot drift: Esri, OpenStreetMap contributors, AISStream.io, USGS, NASA FIRMS (full acknowledgement text), Open-Meteo, GDELT headlines, and the CC BY ship-model credit.
* **SEO**: `app/layout.tsx` carries full metadata (OpenGraph, Twitter card, canonical, JSON-LD SoftwareApplication) plus `sitemap.ts`/`robots.ts` (dashboard and API disallowed). The landing `page.tsx` stays server-rendered with heavy sections behind `next/dynamic`; only `/dashboard/globe` ever imports Cesium.
* **Corridor live stats**: `ports.py` computes `liveVesselCount`/`nearestVesselNm` (+`loiteringCount`/`meanSogKn`) with NumPy vectorized haversine over the cached vessel list, with a pure-Python fallback and None-safe parsing.
* **Degraded boot**: the lifespan connects to Postgres but never dies when the DB is unreachable (it logs and keeps serving). Vessel/hazard proxies and the corridor (static drafts + live counts) stay up; DB-backed endpoints fail per-request instead. The vessel proxy opens a bounded 5s AISStream collect per cache miss: live positions when terrestrial receivers cover the bbox, badged representative demo traffic when the key is missing or no receiver covers the area (empty snapshots never render an empty ocean). So if the globe ever says "Unavailable", check the backend process first: `python -m uvicorn app.main:app --host 127.0.0.1 --port 8000` from `backend/`, with `NEXT_PUBLIC_API_URL` pointing at it. The globe badge shows Connecting while the first poll is in flight, Demo traffic without a key, and the error card prints the API base it tried plus a retry button.
