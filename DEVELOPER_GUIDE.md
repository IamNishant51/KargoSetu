...
* **Degraded boot**: the lifespan connects to Postgres but never dies when the DB is unreachable (it logs and keeps serving). Vessel/hazard proxies and the corridor (static drafts + live counts) stay up; DB-backed endpoints fail per-request instead. So if the globe ever says "Unavailable", check the backend process first: `python -m uvicorn app.main:app --host 127.0.0.1 --port 8000` from `backend/`, with `NEXT_PUBLIC_API_URL` pointing at it. The globe badge shows Connecting while the first poll is in flight, Demo traffic without a key, and the error card prints the API base it tried plus a retry button.
