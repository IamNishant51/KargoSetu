...
const CorridorMiniGlobe = dynamic(() => import("./CorridorMiniGlobe"), {
  ssr: false,
  loading: () => (
    <div className="h-[480px] sm:h-[540px] w-full rounded-2xl bg-[#0A2342] flex flex-col items-center justify-center text-white border border-[#E2E6EB]">
      ...
      <p className="font-mono text-xs uppercase tracking-widest text-[#B45309]">
        Initializing Live Corridor Radar...
      </p>
    </div>
  ),
});
...
export default function PortCorridor() {
  ...
  const hasLive =
    Array.isArray(data) &&
    data.some(
      (p: { liveVesselCount?: number | null }) =>
        typeof p.liveVesselCount === "number",
    );

  return (
    <section
      id="port-corridor"
      className="py-24 sm:py-32 bg-[#F3F5F8] border-y border-[#E2E6EB]"
      ref={containerRef}
    >
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="mt-2 flex flex-col lg:flex-row lg:items-end justify-between gap-3">
          <div>
            ...
            <p className="mt-1 text-sm text-[#3D4F68]">
              Live AIS coastal picture & deep-sea fairway constraints from
              Newcastle to Hooghly.
            </p>
          </div>
        ...
        <div className="mt-8">
          {viewMode === "globe" ? (
            inView ? (
              ...
              <div className="h-[480px] sm:h-[540px] w-full rounded-2xl bg-[#0A2342]" />
            )
          ) : (
            <figure className="card-hard rounded-2xl overflow-hidden bg-white">
              ...
              <figcaption className="flex flex-col sm:flex-row sm:items-center justify-between gap-2 bg-white border-t border-[#E2E6EB] px-5 py-3.5">
                ...
                <span className="text-[12px] font-semibold text-[#0E7A3D]">
                  Haldia 7.5 m · Sandheads 22 m+ · Paradip 14.5 m · Dhamra 16.0
                  m
                </span>
              </figcaption>
            </figure>
                ...
                (p: {
                  ...
                  >
                    <td className="px-5 py-4">
                      <span className="font-mono text-[11px] text-[#6B7D99] mr-2">
                        {p.n}
                      </span>
                      <span className="font-bold text-[15px] text-[#0A2342]">
                        {p.name}
                      </span>
                      <span className="block text-[12.5px] text-[#6B7D99] mt-0.5">
                        {p.sub}
                      </span>
                      {hasLive && typeof p.liveVesselCount === "number" && (
                        <span className="mt-1.5 inline-block font-mono text-[10px] uppercase tracking-[0.12em] bg-[#E9F5EE] text-[#0E7A3D] border border-[#BFE3CD] rounded px-2 py-0.5">
                          {p.liveVesselCount} live · {p.nearestVesselNm ?? "—"}{" "}
                          NM
                        </span>
                      )}
                    </td>
                    <td className="px-5 py-4 font-mono font-semibold text-[15px] text-[#0A2342]">
                      {p.draft}
                    </td>
                    <td className="px-5 py-4 font-mono text-[13px] text-[#0E7A3D]">
                      {p.tide}
                    </td>
                    <td className="px-5 py-4 text-[13.5px] font-semibold text-[#3D4F68]">
                      {p.ship}
                    </td>
                    <td className="px-5 py-4 text-[13px] text-[#3D4F68] max-w-[260px]">
                      {p.note}
                    </td>
                    <td className="px-5 py-4 text-right">
                      <span
                        className={`inline-block font-mono text-[10px] uppercase tracking-[0.12em] px-2 py-1 rounded border ${p.flag === "Watch" ? "bg-[#FDF1E7] text-[#B45309] border-[#F0D3B8]" : "bg-[#E9F5EE] text-[#0E7A3D] border-[#BFE3CD]"}`}
                      >
                        {p.flag}
                      </span>
                    </td>
        ...
        <p className="mt-3 font-mono text-[10.5px] uppercase tracking-[0.14em] text-[#6B7D99]">
          Swipe sideways on mobile · figures from port circulars + live tide
        </p>
      </div>
    </section>
  );
