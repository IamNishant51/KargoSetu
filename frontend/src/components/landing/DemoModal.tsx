...
const DEMO_STEPS = [
  {
    ...
    tag: "Physics Engine",
    subtitle: "Analyzing vessel Under Keel Clearance (UKC) & dynamic tidal draft at Haldia Dock Complex",
    description: "When a 150,000 MT Capesize requisition is entered, KargoSetu's hydrodynamic solver fetches live bathymetry (7.5m permissible draft) and Open-Meteo tide telemetry (+3.2m). Detecting immediate physical grounding risk, it halts direct berthing and calculates feasible offshore operations.",
    telemetry: [
      ...
      { label: "Physics Verdict", val: "INFEASIBLE FOR DIRECT BERTHING", highlight: true }
    ]
  },
  {
    id: 2,
    ...
    subtitle: "Automating 3x Supramax split and Sandheads offshore transshipment protocol",
    description: "Instead of losing the cargo fixture or incurring $35,000/day in demurrage penalties, the constraint engine splits the 150,000 MT cargo into 3x 50,000 MT Supramax vessels. Each lighter vessel maintains a safe 6.8m arrival draft with 1.4m clearance margin.",
    telemetry: [
      ...
      { label: "Demurrage Avoided", val: "$175,000 (5 Days Standstill)", highlight: true }
    ]
  },
  {
    id: 3,
    ...
    subtitle: "Predicting 90-day forward freight curves and detecting optimal CoA booking windows",
    description: "KargoSetu ingests Baltic Dry Index (BDI) and BDRY historical volatility. The neural network generates P10 (Optimistic), P50 (Median), and P90 (Pessimistic) confidence intervals, alerting procurement officers to a 12.8% rate dip in Week 3.",
    telemetry: [
      { label: "Model Architecture", val: "TensorFlow.js Auto-Regressive LSTM" },
      ...
      { label: "Projected PSU Savings", val: "₹4.82 Crores (SAIL Allocation)", highlight: true }
    ]
  },
  {
    id: 4,
    ...
    subtitle: "Minimizing empty sailing legs and calculating IMO-compliant VLSFO emissions",
    description: "To eradicate idle losses ($25,000/day), KargoSetu models a triangular backhaul route (Haldia -> Paradip -> Maputo) rather than returning in ballast. It computes fuel savings and IMO carbon compliance certificates in real-time.",
    telemetry: [
      { label: "Repositioning Strategy", val: "Triangular East Coast Backhaul" },
      ...
      { label: "Fleet Utilization", val: "94.2% (+18% vs Spot Average)", highlight: true }
    ]
  }
];
      ...
      >
        ...
        <div className="flex items-center justify-between px-6 py-4 border-b border-slate-200 bg-slate-50/80">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h3 className="text-base font-bold text-slate-900">KargoSetu Interactive Walkthrough</h3>
              <p className="text-xs text-slate-500">Autonomous Freight Intelligence & Constraint Engine Demo</p>
            </div>
          </div>
        ...
        <div className="overflow-y-auto p-6 space-y-6">
            ...
            {DEMO_STEPS.map((step, idx) => (
              ...
              >
                <span className={`w-4 h-4 rounded-full flex items-center justify-center text-[10px] font-bold ${
                  activeStepIndex === idx ? 'bg-[#EA580C] text-white' : 'bg-slate-200 text-slate-700'
                }`}>
                  {step.id}
                </span>
          ...
          <div className="bg-slate-50 rounded-2xl p-6 text-slate-900 border border-slate-200 shadow-2xs relative overflow-hidden">
            <div className="relative z-10">
              ...
              <h4 className="text-lg sm:text-xl font-bold text-slate-900 mb-1">{currentStep.title}</h4>
              <p className="text-xs sm:text-sm text-slate-500 mb-4">{currentStep.subtitle}</p>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-2.5">
                {currentStep.telemetry.map((item, i) => (
                  ...
                  >
                    ...
                    <div className="font-semibold text-slate-900 text-xs sm:text-sm">{item.val}</div>
                  </div>
                ))}
              </div>
            </div>
        ...
        <div className="flex flex-col sm:flex-row items-center justify-between gap-3 px-6 py-4 border-t border-slate-200 bg-slate-50">
          <div className="flex items-center gap-2 w-full sm:w-auto">
            <button
              onClick={() => setActiveStepIndex((prev) => (prev > 0 ? prev - 1 : DEMO_STEPS.length - 1))}
            ...
            <button
              onClick={() => setActiveStepIndex((prev) => (prev < DEMO_STEPS.length - 1 ? prev + 1 : 0))}
              className="flex-1 sm:flex-initial px-3.5 py-2 rounded-lg border border-slate-300 bg-white text-xs font-semibold text-slate-700 hover:bg-slate-100 transition-colors cursor-pointer"
            >
              Next Step
            </button>
