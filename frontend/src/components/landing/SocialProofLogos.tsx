...
const LOGOS = [
  ...
  },
  {
    name: "MSC",
    icon: (
      <svg viewBox="0 0 24 24" className="w-7 h-7 stroke-current fill-none stroke-2" strokeLinecap="round" strokeLinejoin="round" aria-hidden="true">
  ...
  },
  {
    ...
    customLayout: (
      <div className="flex flex-col text-left leading-none">
        <span className="font-extrabold text-base sm:text-lg tracking-wide font-sans">COSCO</span>
        <span className="font-bold text-[9px] tracking-widest text-slate-500">SHIPPING</span>
      </div>
    ),
  ...
  },
  {
    name: "CMA CGM",
    icon: (
      <svg viewBox="0 0 28 20" className="w-8 h-6 fill-current" aria-hidden="true">
        <path d="M2 14 C6 6, 14 4, 26 5 C20 9, 14 11, 2 14 Z" opacity="0.8" />
        <path d="M4 17 C10 11, 18 10, 26 12 C18 15, 12 16, 4 17 Z" opacity="0.5" />
      </svg>
    ),
  ...
  },
  {
    ...
    textClass: "font-black text-lg sm:text-xl tracking-wide font-sans text-slate-900",
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
        ...
        <div className="hidden md:flex flex-wrap justify-center items-center gap-10 lg:gap-16 opacity-75 grayscale hover:grayscale-0 transition-all duration-300">
          {LOGOS.map((item, idx) => (
            ...
            >
              {item.icon}
              {item.customLayout ? item.customLayout : (
