      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex flex-col sm:flex-row sm:items-end justify-between gap-4 mb-8 sm:mb-10">
          <div>
            <p className="mono-label text-[#B45309]">
              Engine room — four habits
            </p>
            <h2 className="mt-2 font-display font-black text-4xl sm:text-5xl text-[#0A2342]">
              Built like a chief engineer thinks.
            </h2>
          </div>
          <p className="font-mono text-[11.5px] uppercase tracking-[0.14em] text-[#6B7D99]">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 sm:gap-5">
          {FEATURES.map((f) => (
            <article
              key={f.n}
              className="card-hard rounded-2xl bg-[#FAF7F1] p-6 sm:p-8 hover:bg-white transition-colors"
            >
              <div className="flex items-center justify-between gap-3 mb-5">
                <span className="w-9 h-9 rounded-lg bg-[#0A2342] text-white font-display font-black flex items-center justify-center text-lg">
                  {f.n}
                </span>
                <span className="font-mono text-[10px] uppercase tracking-[0.14em] text-[#6B7D99] border border-[#E2E6EB] bg-white rounded px-2 py-1">
                  {f.tag}
                </span>
              </div>
              <h3 className="font-display font-bold text-[26px] sm:text-[28px] text-[#0A2342] leading-tight">
                {f.title}
              </h3>
              <p className="mt-2.5 text-[14.5px] leading-relaxed text-[#3D4F68]">
                {f.body}{" "}
                {"href" in f &&
                  typeof (f as { href?: string }).href === "string" && (
                    <a
              ...
              <ul className="mt-5 pt-4 border-t border-[#E2E6EB] space-y-2">
                {f.metrics.map((m) => (
                  <li
                    key={m}
                    className="flex items-center gap-2 font-mono text-[11px] uppercase tracking-[0.12em] text-[#3D4F68]"
                  >
                    <CheckCircle2
                      size={14}
                      className="text-[#0E7A3D] shrink-0"
                    />{" "}
                    {m}
                  </li>
                ))}
