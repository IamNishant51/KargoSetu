...
const FEATURES: FeatureItem[] = [
  {
    ...
    description:
      "Evaluates vessel Draft, Beam, LOA, and Under Keel Clearance (UKC) against destination bathymetry. Dynamically pulls Open-Meteo tidal curves to ensure ships never run aground in riverine ports like Haldia.",
    metrics: [
      "< 150ms Solver Latency",
      "Dynamic Tidal Compensation",
      "100% Zero-Grounding Guarantee",
    ],
  ...
  },
  {
    ...
    description:
      "Auto-regressive LSTM neural network ingesting Baltic Dry Index (BDI) and historical volatility to output P10, P50, and P90 confidence intervals. Memory-sandboxed via tf.tidy() for zero leak server execution.",
    metrics: [
      "Multi-Horizon (30/60/90 Days)",
      "What-If Volatility Slider",
      "Optimal CoA Window Detection",
    ],
  ...
  },
  {
    ...
    description:
      "When single-vessel routing is physically infeasible due to channel siltation or draft limits, KargoSetu automatically models multi-vessel splitting (e.g. 150k MT to 3x Supramax) with Sandheads deepwater lighterage.",
    metrics: [
      "Auto Cargo Splitting",
      "Offshore Lighterage Protocol",
      "Demurrage Penalty Elimination",
    ],
  ...
  },
  {
    ...
    description:
      "Eradicates $25,000/day idle ballast losses through intelligent triangular repositioning fixtures. Automatically computes VLSFO bunker burn and IMO-compliant CO2 footprint savings.",
    metrics: [
      "Triangular Backhaul Optimization",
      "VLSFO Fuel Burn Minimization",
      "IMO Decarbonization Footprint",
    ],
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="text-center max-w-3xl mx-auto mb-16">
          ...
          <p className="text-slate-500 text-base sm:text-lg font-medium leading-relaxed">
            Merging naval architecture hydrodynamics with auto-regressive deep
            learning to solve India&apos;s bulk logistics challenges.
          </p>
        </div>
          ...
          {FEATURES.map((item, idx) => {
            ...
            return (
              ...
              >
                {/* Content Side */}
                <div
                  className={`relative z-10 flex flex-col justify-between h-full ${isLarge ? "p-8 sm:p-10 md:w-1/2" : ""}`}
                >
                  <div>
                    <div className="flex items-center justify-between gap-4 mb-6">
                      <div
                        className={`w-12 h-12 rounded-2xl flex items-center justify-center border ${item.accent}  transition-transform shadow-sm`}
                      >
                  ...
                  <div className="pt-6 border-t border-slate-100">
                    <div
                      className={`grid gap-3 ${isLarge ? "grid-cols-1 sm:grid-cols-3" : "grid-cols-1"}`}
                    >
                      {item.metrics.map((metric, mIdx) => (
                        <div
                          key={mIdx}
                          className="flex items-center gap-2 text-[10px] font-bold text-slate-700 tracking-wider uppercase"
                        >
                          <CheckCircle2
                            size={14}
